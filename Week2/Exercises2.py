...
#%%
def whoistheretotrade(player):
    if "Michael Jordan" in player:
        print("MJ is available.")
    if "Kobe" in player:
        print("Kobe is available.")
    if "Carmelo" in player or "Dwayne Wade" in player:
        print("There are 2nd tier players.")
    if "Stef Curry" in player or "Ray Allen" in player:
        print("There are sharp shooters.")    
    if "Stef Curry" in player and "Ray Allen" in player:
        print("There are at least 2 sharp shooters.")
    if "Lebron" not in player:
        print("Lebron is not available")
    print("There are", len(player), "in list.")
...
def average(numlis):
    ...
    for i in numlis:
        ...
        count = count + 1
    average = sum/count    
    print("average is", average,"its count is", count)  
...
def average(nlis):
    sum_ = 0
    for number in range(0,len(nlis)):
        print(nlis[number],end = " ")
        sum_ = sum_ + nlis[number]
    avg = sum_ / len(nlis)
    print() 
